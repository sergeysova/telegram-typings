...
const {
	...
	TypeName, TypeVariants, TypeArray, TypeProperty, TypeStructure, TypeAny
...
class ParserUlList {
	...
	parse() {
		...
		if ( !liArray.every(isTypeName) )
			throw new Error(`Invalid parse data`);
	...
	parse(window) {
		...
		const types = new Map(
			...
			.map(el => {
				...
				if ( table )
					return [name, new TypeStructure((new ParserTableType(table)).parse(), comments)];
				
				const ul = $nextTag(el, "ul", "h4");
				if ( ul )
					return [name, new TypeVariants((new ParserUlList(ul)).parse() , comments)];
