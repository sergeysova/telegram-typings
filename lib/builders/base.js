...
class BaseBuilder {
   ...
   */
  buildInterface(interf/*: Interface*/) { // eslint-disable-line no-unused-vars
   ...
   */
  buildModule(interfaces/*: mixed[]*/)/*: mixed[]*/ { // eslint-disable-line no-unused-vars
  ...
  iterateOverTypes() {
    return [...this.store.models()]
      .filter((interf) => !interf.native)
      .map((interf) => {
        if (interf instanceof Interface) {
          return this.buildInterface(interf)
        }
        if (interf instanceof Union) {
          const notFound = interf.variants.filter((name) => !this.store.has(name))

          if (notFound.length > 0) {
            throw new TypeError(`${notFound[0]} not found in Store`)
          }
          return this.buildUnion(interf)
        }
