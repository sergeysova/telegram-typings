const { Interface, Method, Union /*::, Store, */ } = require('../store')
...
class BaseBuilder {
  ...
  buildMethod(object/*: Method*/) { // eslint-disable-line no-unused-vars
    throw new TypeError('The `buildMethod` method should be overriden')
  }
    ...
    return [...this.store.iterate()]
      ...
      .map((object) => {
        ...
        if (object instanceof Method) {
          return this.buildMethod(object)
        }
  ...
  buildNativeType(name/*: string | Array<string>*/) {
    ...
    if (name.includes(',')) {
      return this.buildUnionOfTypes(name.split(',').map((s) => s.trim()))
    }
