...
const getElementsByTextRegex = (nodes, regex) => [...nodes]
  .filter(async (node) => {
    const text = await node.getProperty('innerText')
...
(async () => {
  ...
  let methods = [...nodes].filter(async (node) => {
    const text = await node.getProperty('innerText')
    const value = await text.jsonValue()
  ...
  types = await Promise.all(types.map(async (element) => {
    ...
    while (next.nextElementSibling.tagName === 'P') {
      ...
      comment += await (await next.getProperty('innerText')).jsonValue()
    }
    ...
    if (table) {
      properties = [...table.children[1].children].map((tr) => {
        ...
        return {
          type: arr[1].innerText,
          name: arr[0].innerText,
          required: !arr[2].innerText.startsWith('Optional.'),
          comment: arr[2].innerText,
        }
      })
    }
    ...
    if (table) {
      params = [...table.children[1].children].map((tr) => {
        ...
        return {
          type: arr[1].innerText,
          name: arr[0].innerText,
          required: arr[2].innerText === 'Yes',
          comment: arr[3].innerText,
        }
      })
    }
