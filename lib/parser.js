  ...
  for (const table of list) {
    ...
    if (type === 'Field' || type === 'Parameters') {
      ...
      for (const row of rows) {
        const [flName, flType, flDescrOrRequired, flDescrOrUndef] = await toArray(row.find('td'))
        let fieldDescription
        let optional = false

        if (flDescrOrUndef) {
          fieldDescription = flDescrOrUndef.text().trim()
          optional = fieldDescription.trim() === 'Optional'
        }
        else {
          fieldDescription = flDescrOrRequired.text().trim()
          if (fieldDescription.includes('Optional.')) {
            optional = true
            fieldDescription = fieldDescription.replace('Optional.', '').trim()
          }
        }
        const links = parseLinks(flDescrOrUndef || flDescrOrRequired)
