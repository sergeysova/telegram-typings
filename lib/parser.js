...
const { Interface, Method, Field } = require('./store')
...
async function run() {
  ...
  types = await Promise.all(types.map(async (element) => {
    ...
    while (await getPropertyValue(await getPropertyValue(next, 'nextElementSibling'), 'tagName') === 'P') {
      ...
      description += await getPropertyValue(next, 'textContent')
    }
    ...
    if (table) {
      fields = await Promise.all((await getPropertyValue((await getPropertyValue(table, 'children'))[1], 'children')).map(async (tr) => {
        const array = await getPropertyValue(tr, 'children')

        return new Field(
          await getPropertyValue(array[0], 'textContent'),
          await getPropertyValue(array[1], 'textContent'),
          {
            optional: (await getPropertyValue(array[2], 'textContent')).startsWith('Optional.'),
            description: await getPropertyValue(array[2], 'textContent'),
          },
        )
      }))
    }
    ...
    return new Interface(
      await getPropertyValue(element, 'textContent'),
      { description },
      fields,
    )
  }))
    ...
    if (table) {
      fields = await Promise.all((await getPropertyValue((await getPropertyValue(table, 'children'))[1], 'children')).map(async (tr) => {
        ...
        return new Field(
          ...
          await getPropertyValue(array[1], 'textContent'),
          {
            optional: await getPropertyValue(array[2], 'textContent') !== 'Yes',
            description: await getPropertyValue(array[3], 'textContent'),
          },
        )
      }))
    }
